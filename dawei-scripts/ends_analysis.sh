...
##### Ben's additions for running on Berkeley server:
...
if [ "$mode" == "regions" ]; then
	for ((i=0;i<${#regionarray[@]};++i)); do
		...
		python2 "$path_to_scripts"/ends_analysis_get_intervals.py "${regionarray[i]}" "${outprefix}_${namearray[i]}.bed" "$width" "$numIn" "$numOut" > "${outprefix}_LSF_logs/ends_analysis_get_intervals_${namearray[i]}.txt"
	...
	python2 "$path_to_scripts"/ends_analysis_get_intervals.py "${regionarray[0]}" "${outprefix}_regions.bed" "$width" "$numIn" "$numOut"
...
#	awk -F$'\t' 'BEGIN{ OFS = FS } { tt[$7]+=1; ss[$7]+=$8 } END { for (id in tt) {print id,ss[id]/tt[id]} } ' "${outprefix}_${namearray[i]}_intersect.bed" | sort -k1n,1 > "${outprefix}_${namearray[i]}_counts.txt"
	...
	python2 $path_to_scripts/ends_analysis_process_intersect.py "${outprefix}_${namearray[i]}_intersect.bed" "${outprefix}_${namearray[i]}_counts.txt"${weight}	
done
echo ""
...
if "$writemat"; then
	...
	#		echo "Converting results of step 2 into matrix format for ${namearray[i]}..."
			cmd="python2 $path_to_scripts/ends_analysis_make_matrix.py ${outprefix}_${namearray[i]}_intersect.bed ${outprefix}_${namearray[i]}_mat.txt $numIn $numOut $width"
		...
		for ((i=0;i<${#namearray[@]};++i)); do
			echo "Converting results of step 2 into matrix format for ${namearray[i]}..."
			python2 $path_to_scripts/ends_analysis_make_matrix.py "${outprefix}_${namearray[i]}_intersect.bed" "${outprefix}_${namearray[i]}_mat.txt" $numIn $numOut $width
		done
	fi
	echo ""
fi
...
if [ -z "$colors" ]; then
	$path_to_scripts/ends_analysis_make_plot.R "${outprefix}_average_counts_filt.txt" "$yupper" "$ylabel" "$title" "$outprefix" "$numIn" "$numOut" "$width" "$linewidth"${bb} > "${outprefix}_LSF_logs/make_plot.txt"
else
	$path_to_scripts/ends_analysis_make_plot.R "${outprefix}_average_counts_filt.txt" "$yupper" "$ylabel" "$title" "$outprefix" "$numIn" "$numOut" "$width" "$linewidth" --colors "$colors"${bb} > "${outprefix}_LSF_logs/make_plot.txt"
fi
[ $? != 0 ] && { echo "Error: ends_analysis_make_plot failed, see ${outprefix}_LSF_logs/make_plot.txt"; exit 1; }
