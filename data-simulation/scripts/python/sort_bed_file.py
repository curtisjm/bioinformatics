...
def sort_directory(in_dir, out_dir):
    ...
    for file in files:
        if file.endswith(".bed"):
            bed_data = pd.read_csv(in_dir + file, sep="\t", names=col_labels, header=None)
...
def sort_single_file(file):
    ...
    bed_data = bed_data.sort_values(by=["chr", "start"])
    bed_data.to_csv(f"{os.path.basename(file).replace('.bed', '')}_sorted.bed", sep="\t", header=False, index=False)

sort_single_file("/Users/curtis/Documents/bioinformatics/data-simulation/real-data/4-4_R0_all_CHH.bed")